...
def _search_directory(dname, token, type_check_fun, search_fun):
...
def main(name, token, type_check_fun, search_fun):
	if os.path.isfile(name):
		...
		_search_directory(name, token, type_check_fun, search_fun)
	else:
		raise TypeError("Must be a directory or file.")


def object_main(name, token):
	main(name, token, _is_object_file, _nm_search)


def source_main(name, token):
	main(name, token, _is_c_source, _cat_search)


def header_main(name, token):
	main(name, token, _is_c_header, _cat_search)
...
if __name__ == '__main__':
	...
	if o_mode:
		object_main(directory, symbol)
	if s_mode:
		source_main(directory, symbol)
	if h_mode:
		header_main(directory, symbol)
